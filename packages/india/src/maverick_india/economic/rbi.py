...
class RBIDataProvider:
    ...
    def __init__(
        ...
        Args:
            ...
            trading_economics_api_key: Optional Trading Economics API key
            cache_ttl: Cache time-to-live in seconds (default: 1 hour)
        """
        self.api_key = api_key
        self.trading_economics_api_key = trading_economics_api_key

        # Cache with configurable TTL
        self.cache: TTLCache[str, Any] = TTLCache(maxsize=100, ttl=cache_ttl)
    ...
    def get_policy_rates(self) -> dict[str, Any]:
        ...
        try:
            ...
            if not rates:
                ...
                logger.warning(
                    "Using approximate policy rates - real-time data not available"
                )
                rates = {
    ...
    def _fetch_policy_rates_from_rbi(self) -> dict[str, Any] | None:
        """
    ...
    def get_inflation_data(self, period: str = "1y") -> pd.DataFrame:
        ...
        try:
            ...
            df = self._fetch_world_bank_indicator(
                "FP.CPI.TOTL.ZG", start_date.year, end_date.year  # CPI inflation
            )
    ...
    def get_gdp_growth(self) -> dict[str, Any]:
        ...
        try:
            ...
            if df.empty:
                return {"error": "GDP data not available", "status": "error"}
    ...
    def get_forex_reserves(self) -> dict[str, Any]:
        ...
        try:
            ...
            if df.empty:
                return {"error": "Forex reserves data not available", "status": "error"}

            latest_data = df.iloc[-1]

            forex_data: dict[str, Any] = {
                "total_reserves_usd": float(latest_data["value"])
    ...
    def _fetch_world_bank_indicator(
        self, indicator_code: str, start_year: int, end_year: int
    ) -> pd.DataFrame:
        """
