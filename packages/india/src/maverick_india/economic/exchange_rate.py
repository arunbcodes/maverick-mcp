...
class ExchangeRateProvider:
    ...
    def __init__(
        ...
        cache_ttl: int = 3600,
        default_usd_inr_rate: float = 83.0,
    ):
        """
        ...
        Args:
            ...
            default_usd_inr_rate: Default USD/INR rate for fallback
        """
        self.api_key = api_key
        self.use_fallback = use_fallback

        # Cache with configurable TTL (rates don't change frequently)
        self.cache: TTLCache[str, dict[str, Any]] = TTLCache(maxsize=100, ttl=cache_ttl)

        # Default approximate rate (used as last resort)
        self.default_usd_inr_rate = default_usd_inr_rate
    ...
    def _fetch_from_exchange_rate_api(
        ...
        try:
            ...
            if data.get("result") == "success":
                ...
                logger.warning(
                    f"Exchange Rate API returned error: {data.get('error-type')}"
                )
                return None
    ...
    def _get_approximate_rate(
        ...
        if from_currency == "USD" and to_currency == "INR":
            ...
            logger.error(
                f"No approximate rate available for {from_currency}/{to_currency}"
            )
    ...
    def is_rate_stale(
        self, rate_data: dict[str, Any], max_age_hours: int = 24
    ) -> bool:
        """
...
def get_exchange_rate(
    ...
    return result.get("rate", 0.0) or 0.0
