...
from maverick_india.news.economic_times import EconomicTimesScraper
from maverick_india.news.moneycontrol import MoneyControlScraper
...
class MultiSourceNewsAggregator:
    ...
    def __init__(
        ...
        self.moneycontrol = MoneyControlScraper(
            article_store=article_store, cache_ttl=cache_ttl
        )
        self.economic_times = EconomicTimesScraper(
            article_store=article_store, cache_ttl=cache_ttl
        )
    ...
    def fetch_stock_news(
        ...
        if "moneycontrol" in sources:
            try:
                ...
                all_articles.extend(mc_articles)
                logger.info(
                    f"Fetched {len(mc_articles)} MoneyControl articles for {symbol}"
                )
            except Exception as e:
        ...
        logger.info(
            f"Returning {len(sorted_articles[:limit])} deduplicated articles for {symbol}"
        )
        return sorted_articles[:limit]
    ...
    def get_sentiment_summary(
        ...
        by_source: dict[str, dict[str, int]] = defaultdict(
            lambda: {"count": 0, "bullish": 0, "bearish": 0, "neutral": 0}
        )
    ...
    def get_trending_topics(self, limit: int = 10) -> list[dict[str, Any]]:
        ...
        keyword_articles: dict[str, list[str]] = defaultdict(list)
...
def get_aggregated_news(
    ...
    """
    aggregator = MultiSourceNewsAggregator(article_store=article_store)
