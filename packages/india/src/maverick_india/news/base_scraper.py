...
and caching. Specific news sources only need to implement source-specific methods.

This module is designed to be independently deployable. Database persistence
is optional and can be injected via the NewsArticleStore protocol.
"""
...
@runtime_checkable
class NewsArticleStore(Protocol):
    """Protocol for news article storage."""
...
class BaseNewsScraper(ABC):
    ...
    def __init__(
        ...
        self.cache: TTLCache[str, list[dict[str, Any]]] = TTLCache(
            maxsize=100, ttl=cache_ttl
        )
        self.session = requests.Session()

        # Setup session with user agent
        self.user_agent = (
            user_agent or "MaverickMCP/1.0 (Financial News Aggregator)"
        )
        self.session.headers.update({"User-Agent": self.user_agent})

        logger.info(
            f"{self.__class__.__name__} initialized "
            f"(DB: {article_store is not None}, Cache TTL: {cache_ttl}s, Source: {self.get_source_name()})"
        )
    ...
    def fetch_latest_news(
        ...
        if use_cache and cache_key in self.cache:
            logger.debug(
                f"Returning cached news for {category} from {self.get_source_name()}"
            )
        ...
        try:
            ...
            if not feed.entries:
                logger.warning(
                    f"No entries found in {category} feed from {self.get_source_name()}"
                )
                return []
            ...
            logger.info(
                f"Fetched {len(articles)} articles from {category} ({self.get_source_name()})"
            )
            ...
            logger.error(
                f"Error fetching news from {category} ({self.get_source_name()}): {e}"
            )
            return []
    ...
    def fetch_stock_news(
        ...
        if cache_key in self.cache:
            logger.debug(
                f"Returning cached news for {symbol} from {self.get_source_name()}"
            )
            return self.cache[cache_key]
        ...
        if self.article_store:
            db_articles = self.article_store.get_recent_articles(
                symbol=symbol,
        ...
        logger.info(
            f"Found {len(stock_articles)} articles for {symbol} from {self.get_source_name()}"
        )
    ...
    def scrape_article_content(self, url: str) -> str | None:
        ...
        try:
            ...
            logger.warning(
                f"Could not extract content from {url} ({self.get_source_name()})"
            )
            return None
    ...
    def _parse_rss_entry(
        ...
        try:
            ...
            logger.error(
                f"Error parsing RSS entry from {self.get_source_name()}: {e}"
            )
            return None

    def _store_article(self, article: dict[str, Any]) -> None:
        """Store article using the article store."""
        if not self.article_store:
            return

        try:
            self.article_store.store_article(
                title=article["title"],
                url=article["url"],
            ...
            logger.error(f"Error storing article ({self.get_source_name()}): {e}")
    ...
    def _get_company_name(self, symbol: str) -> str | None:
        ...
        Uses the IndianStockSymbolMapper for symbol resolution.
        """
...
__all__ = ["BaseNewsScraper", "NewsArticleStore"]
