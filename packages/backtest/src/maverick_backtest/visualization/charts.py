"""
Backtesting Visualization - Chart Generation.

Provides chart generation utilities for backtest results including:
...
def set_chart_style(theme: str = "light") -> None:
    ...
    Args:
        theme: Chart theme, either 'light' or 'dark'
    """
    # Use a style that's available in modern matplotlib
...
def generate_equity_curve(
    ...
    Args:
        returns: Cumulative returns series
...
def generate_trade_scatter(
    ...
    Args:
        prices: Price series
        trades: Trades DataFrame with entry/exit points (columns: type, price)
        title: Chart title
        theme: Chart theme
...
def generate_optimization_heatmap(
    ...
    Args:
        param_results: Dictionary of parameter combinations and performance
        title: Chart title
        theme: Chart theme
...
def generate_portfolio_allocation(
    ...
    Args:
        allocations: Dictionary of symbol allocations (symbol -> weight)
        title: Chart title
        theme: Chart theme
...
def generate_strategy_comparison(
    ...
    Args:
        strategies: Dictionary of strategy returns (strategy_name -> returns_series)
        title: Chart title
        theme: Chart theme
...
def generate_returns_distribution(
    ...
    try:
        ...
        ax.axvline(
            ...
            color="orange",
            linestyle=":",
            label=f"+1 Std: {mean_return + std_return:.4f}",
        )
        ax.axvline(
            mean_return - std_return,
            color="orange",
...
def generate_rolling_metrics(
    ...
    """
    Generate rolling performance metrics chart (Sharpe, Volatility).
    ...
    try:
        ...
        # Plot rolling Sharpe
        rolling_sharpe.plot(ax=ax1, linewidth=2, color="blue")
        ax1.set_title(f"Rolling {window}-Day Sharpe Ratio")
        ax1.set_ylabel("Sharpe Ratio")
        ax1.axhline(0, color="black", linestyle="--", alpha=0.5)
        ax1.grid(True, linestyle="--", alpha=0.7)

        # Plot rolling volatility
        rolling_std.plot(ax=ax2, linewidth=2, color="red")
        ...
        logger.error(f"Error generating rolling metrics: {e}")
        return ""
