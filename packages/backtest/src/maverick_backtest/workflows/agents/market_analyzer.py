...
class MarketAnalyzerAgent:
    ...
    def __init__(
        ...
        cache_manager: CacheProtocol | None = None,
    ):
    ...
    async def _get_price_data(
        ...
        if not self.data_provider:
            logger.warning("No data provider configured for MarketAnalyzerAgent")
            return None
    ...
    def _analyze_trend(self, close: pd.Series) -> dict[str, Any]:
        ...
        # Create proper OHLC DataFrame for ADX
        high_df = close.to_frame(name="high")
        low_df = close.to_frame(name="low")
        adx_result = ta.adx(high_df["high"], low_df["low"], close, length=14)
        adx = (
            adx_result.iloc[-1, 0]
