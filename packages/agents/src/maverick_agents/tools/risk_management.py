...
Uses Protocol-based dependency injection for stock data and technical analysis
to maintain clean package boundaries.
"""
...
from maverick_agents.personas import PersonaAwareTool
...
@runtime_checkable
class StockDataProviderProtocol(Protocol):
...
class TechnicalAnalysisProtocol(Protocol):
    """Protocol for technical analysis functions."""
...
# Default ATR calculation (fallback if no provider injected)
def _default_calculate_atr(df: pd.DataFrame, period: int = 14) -> pd.Series:
    """Calculate Average True Range using standard formula."""
    high = df["High"]
    low = df["Low"]
    close = df["Close"]
...
class TechnicalStopsTool(PersonaAwareTool):
    ...
    _stock_provider: StockDataProviderProtocol | None = None
    _atr_calculator: TechnicalAnalysisProtocol | None = None

    def set_stock_provider(self, provider: StockDataProviderProtocol) -> None:
        """Inject stock data provider."""
        self._stock_provider = provider

    def set_atr_calculator(self, calculator: TechnicalAnalysisProtocol) -> None:
        """Inject ATR calculator."""
        self._atr_calculator = calculator

    def _calculate_atr(self, df: pd.DataFrame, period: int = 14) -> pd.Series:
        """Calculate ATR using injected calculator or default."""
        if self._atr_calculator:
            return self._atr_calculator.calculate_atr(df, period)
        return _default_calculate_atr(df, period)
    ...
    def _run(
        ...
        try:
            if not self._stock_provider:
                return "Error: Stock data provider not configured. Use set_stock_provider() first."
            ...
            df = self._stock_provider.get_stock_data(
                symbol,
...
class RiskMetricsTool(PersonaAwareTool):
    ...
    def _run(
        ...
        try:
            ...
            spy_df = self._stock_provider.get_stock_data(
                "SPY",
                start_date.strftime("%Y-%m-%d"),
                end_date.strftime("%Y-%m-%d"),
                use_cache=True,
