"""
Progressive token budgeting for research agents.

Manages token budgets across research phases with time awareness and
adaptive allocation based on confidence levels.
"""
...
class ProgressiveTokenBudgeter:
    ...
    def allocate_tokens_for_phase(
        ...
        base_timeout = min(time_remaining * 0.8, 45)
        adjusted_timeout = base_timeout * (final_budget / base_budget) ** 0.5

        return TokenAllocation(
            input_tokens=min(int(final_budget * 0.75), 15000),
            output_tokens=min(int(final_budget * 0.25), 3000),
            per_source_tokens=final_budget // max(sources_count, 1)
            if sources_count > 0
            else final_budget,
            emergency_reserve=200,
            timeout_seconds=max(adjusted_timeout, 5),
        )
    ...
    def get_next_allocation(
        ...
        return {
            "time_budget": min(time_per_source, 30.0),
