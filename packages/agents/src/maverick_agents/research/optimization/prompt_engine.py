"""
Optimized prompt engineering for research agents.

Creates optimized prompts for different time constraints and confidence levels
with adaptive templates and caching.
"""
...
Tailor specifically for {persona} investment characteristics.""",
    ...
    def get_optimized_prompt(
        ...
        **context: Any,
        ...
        cache_key = (
            f"{prompt_type}_{time_remaining:.0f}_{confidence_level:.1f}_"
            f"{hash(str(sorted(context.items())))}"
        )
        ...
        if confidence_level > 0.7:
            confidence_instructions = (
                "\n\nNOTE: High confidence already achieved. "
                "Focus on validation and contradictory evidence."
            )
        elif confidence_level < 0.4:
            confidence_instructions = (
                "\n\nNOTE: Low confidence. "
                "Look for strong supporting evidence to build confidence."
            )
    ...
    def create_time_optimized_synthesis_prompt(
        ...
        for source in sources:
            ...
            insights.extend(analysis.get("insights", [])[:2])

        context = {
            "persona": persona,
            "source_count": len(sources),
            "insights": "; ".join(insights[:8]),
            "key_points": "; ".join(insights[:8]),
