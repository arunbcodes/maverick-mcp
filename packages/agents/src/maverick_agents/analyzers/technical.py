...
Provides comprehensive technical analysis for stocks including:
...
class TechnicalAnalysisAgent(PersonaAwareAgent):
    ...
    def _get_tools_from_registry(
        ...
        """Get technical analysis tools from registry."""
        tool_names = [
        ...
        if not tools:
            logger.warning("No technical tools available from registry, using mock tools")
    ...
    async def _construct_trade_setup(
        ...
        try:
            ...
            if current_price > 0:
                ...
                # Get stop loss recommendation from tools
                stops_tool = next(
                    (t for t in self.tools if "stop" in t.name.lower()), None
                )
