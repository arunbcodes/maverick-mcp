...
# Validation defaults (can be overridden by configuration)
DEFAULT_MIN_SYMBOL_LENGTH = 1
DEFAULT_MAX_SYMBOL_LENGTH = 20

# Custom type annotations
TickerSymbol = Annotated[
    str,
    Field(
        min_length=DEFAULT_MIN_SYMBOL_LENGTH,
        max_length=DEFAULT_MAX_SYMBOL_LENGTH,
        pattern=r"^[A-Z0-9\-\.]{1,20}$",
        description="Stock ticker symbol (e.g., AAPL, BRK.B, SPY, RELIANCE.NS)",
    ),
]
...
class TickerValidator:
    ...
    def validate_ticker(
        value: str,
        min_length: int = DEFAULT_MIN_SYMBOL_LENGTH,
        max_length: int = DEFAULT_MAX_SYMBOL_LENGTH,
    ) -> str:
        """Validate and normalize ticker symbol."""
        # Convert to uppercase
        ticker = value.upper().strip()

        if len(ticker) < min_length:
            raise ValueError(
        ...
        # Check pattern
        pattern = f"^[A-Z0-9\\-\\.]{{1,{max_length}}}$"
        if not re.match(pattern, ticker):
            raise ValueError(
                f"Invalid ticker symbol: {value}. "
                f"Must be {min_length}-{max_length} characters, alphanumeric with optional . or -"
            )
    ...
    def validate_ticker_list(
        ...
        for ticker in values:
            normalized = TickerValidator.validate_ticker(ticker, min_length, max_length)
