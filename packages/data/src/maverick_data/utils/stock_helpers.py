"""
Stock data helper utilities.

This module provides common stock data fetching and processing utilities
that are shared across multiple components to avoid code duplication.
"""
...
from maverick_data.providers.stock_data import StockDataProvider
...
def get_stock_dataframe(ticker: str, days: int = 365) -> pd.DataFrame:
    ...
    # Try to add technical indicators if available
    try:
        from maverick_data.technical import add_technical_indicators
        df = add_technical_indicators(df)
    except ImportError:
        pass  # Technical indicators not available
