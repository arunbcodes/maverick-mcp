...
def validate_symbol(
    ...
    if pattern and not re.match(pattern, symbol.upper()):
        return False, f"Symbol does not match required pattern"
...
def validate_date_range(
    ...
    """
    from maverick_mcp.utils.datetime_utils import to_date, days_between
...
def validate_not_empty(value: str, field_name: str = "Value") -> tuple[bool, Optional[str]]:
    """
...
def validate_currency_code(code: str) -> tuple[bool, Optional[str]]:
    ...
    known_codes = {
        ...
        "INR", "CNY", "HKD", "SGD", "KRW", "MXN", "BRL", "ZAR"
    }
    
    if code not in known_codes:
        logger.warning(f"Currency code '{code}' is not in known list (but format is valid)")
